    ...
    def get_posterior_rate(self, long_li, short_li, dfs):
        ...
        for i in range(len(long_li)):
            ...
            for code_tem in long_tem:
                ...
                if temp.empty != True:
                    long_temp_rate = long_temp_rate + (temp[-1]/temp[0] - 1)
                else:
                    len_code = len_code -1 
            ...
            temp_rate = 0
            # short_temp_rate2 = 0
            for code_tem2 in short_tem:

                ind2 = self.file_list.index(code_tem2+'.xlsx')
                temp2 = dfs[ind2][((dfs[ind2].index.month == date.month))& (dfs[ind2].index.year == date.year)]['종가(원)']
                # |(dfs[ind2].index.month == date.month-1)|(dfs[ind2].index.month == date.month-2)
                if temp2.empty != True:
                    short_temp_rate = short_temp_rate - (temp2[-1]/temp2[0] - 1)
                else:
                    len_code2 = len_code2 -1 

            if (len_code+len_code2) == 0:
                temp_rate = 0
            else:
                temp_rate = (long_temp_rate + short_temp_rate)/(len_code+len_code2) #(long_temp_rate)/(len_code) + (short_temp_rate)/(len_code2)
            ...
            if len_code == 0:
                ...
                long_temp_rate2 = long_temp_rate/len_code

            # if len_code2 == 0:
            #     short_temp_rate2 = 0
            # else:
            #     short_temp_rate2 = short_temp_rate/len_code2
            
            # rate_sum.append(long_temp_rate2+short_temp_rate2)

            long_rate_sum.append(long_temp_rate2)

            print(f'{i}번째 읽어옴')     

        return rate_sum, long_rate_sum
