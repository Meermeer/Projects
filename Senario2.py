...
class Singularity:
    ...
    def get_month_df(self, df):
        ...
        for k, file in enumerate(self.file_list):
            ...
            code1 = pd.read_excel(f'./KOSPI200_종목코드/{file}')    # 375500 000810
            ...
            date2 =  datetime.strptime('2007-06-01', '%Y-%m-%d')
            code_col_list = code1.columns
            ...
            for i in range(64):
                ...
                for j, col in enumerate (code_col_list):
                    temp = code1[((code1.index.month == date2.month)|(code1.index.month == date2.month-1)|(code1.index.month == date2.month-2)) & (code1.index.year == date.year)][col]
                    # ((code1.index.month == date.month)|(code1.index.month == date.month-1)|(code1.index.month == date.month-2))
                ...
                # date_list3.append(date-relativedelta(months=3))
                date = date+relativedelta(months=3)
                date2 = date2+relativedelta(months=3)
            ...
            df_code = df.loc[df['거래소코드']== file.replace('.xlsx','')]
    ...
    def get_posterior_rate(self, long_li, short_li, dfs):
        ...
        for i in range(len(long_li)):
            ...
            for code_tem in long_tem:
                ...
                # temp_date1 = date + relativedelta(months=2)
                # temp_date2 = date + relativedelta(months=3)
                # temp_date3 = date + relativedelta(months=4)
                ...
                # temp = dfs[ind][((dfs[ind].index.month == temp_date1.month)|
                #                  (dfs[ind].index.month == temp_date2.month)|(dfs[ind].index.month == temp_date3.month))& 
                #                  ((dfs[ind].index.year == temp_date1.year)|(dfs[ind].index.year == temp_date2.year)|(dfs[ind].index.year == temp_date3.year))]['종가(원)']
